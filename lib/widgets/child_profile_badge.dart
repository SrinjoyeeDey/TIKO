...
class _ChildProfileBadgeState extends State<ChildProfileBadge> {
  ...
  void _showDifficultyDialog(BuildContext context, ChildProfile profile) {
    final diffPct = profile.difficultyPercentage;
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<ChildProfile?>(
      valueListenable: ChildState.instance.activeProfileNotifier,
      builder: (context, activeProfile, _) {
        final profile = _profile ?? activeProfile ?? ChildState.instance.currentProfile;
        String name = profile.name;
        if (name.isEmpty || name == 'Parent User' || name == 'Child') {
          final parent = ChildState.instance.currentParent;
          if (parent != null && parent.name.isNotEmpty && parent.name != 'Parent User') {
            name = parent.name;
          } else if (parent != null && parent.email.isNotEmpty) {
            final emailName = parent.email.split('@').first.replaceAll(RegExp(r'[^a-zA-Z0-9]'), '');
            name = emailName.isNotEmpty ? (emailName[0].toUpperCase() + emailName.substring(1)) : 'Explorer';
          } else {
            name = 'Explorer';
          }
        }
